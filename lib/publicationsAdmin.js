...
export function normalizePublicationPayload(raw, options = {}) {
  ...
  if (filePathProvided) {
    ...
    if (!unchanged && filePath) {
      ...
      // New direct TUS uploads are stored under `<auth-user-uuid>/YYYY/<uuid>.<ext>`
      // so Storage RLS can restrict browser uploads to the signed-in account.
      // Older `YYYY/<uuid>.<ext>` paths remain valid for existing publications.
