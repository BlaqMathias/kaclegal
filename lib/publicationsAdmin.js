...
export function normalizePublicationPayload(raw, options = {}) {
  ...
  if (filePathProvided) {
    ...
    if (!unchanged && filePath) {
      ...
      // Storage keys are minted by the upload route as `YYYY/<uuid>.<ext>`. Anything
      // else — an absolute URL, a traversal attempt — is rejected outright.
      if (!/^\d{4}\/[a-f0-9-]{36}\.[a-z0-9]{2,5}$/i.test(filePath)) {
  ...
  if (imagePathProvided) {
    ...
    if (!unchangedImage && imagePath) {
      ...
      // Image keys are minted by the upload-image route as
      // `/images/publicationUploads/<uuid>.<ext>`. Anything else is rejected.
      if (!/^\/images\/publicationUploads\/[a-f0-9-]{36}\.(jpe?g|png|webp|gif)$/i.test(imagePath)) {
