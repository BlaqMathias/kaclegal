...
/**
 * Download a stored file's bytes directly (server-side), rather than minting
 * a separately-reusable signed URL.
 *
 * This exists specifically for the paid-download route: a signed URL, once
 * handed to a browser, keeps working for its own TTL regardless of anything
 * this app tracks — including `download_tokens.downloads_remaining`. Fetching
 * the bytes here and returning them directly means the ONLY way to ever get
 * this file is by hitting the route that counts a download first. No second,
 * uncounted door is ever created.
 *
