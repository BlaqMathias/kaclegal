...
/** App-level ceiling on uploaded publication files: 50 MB (Supabase Free). */
...
/** App-level ceiling on uploaded publication cover images: 20 MB. */
export const MAX_IMAGE_UPLOAD_BYTES = 20 * 1024 * 1024;

/**
 * Browser-displayable image formats we accept for publication covers.
 *
 * The file picker and server both restrict covers to the five supported formats.
 * The server also validates the real bytes before optimization.
 */
...
/**
 * Canonical extension to store for an uploaded image MIME type.
 *
 * Multiple MIME types can legitimately map to the same extension (`jpg`, `tif`).
 */
...
export const IMAGE_UPLOAD_ACCEPT = '.jpg,.jpeg,.png,.webp,.avif,.tif,.tiff,image/jpeg,image/png,image/webp,image/avif,image/tiff';
...
export function extensionFromFilename(filename) {
  const match = /\.([a-z0-9]{2,10})$/i.exec(String(filename ?? '').trim());
...
/**
 * Resolve the extension a cover-image filename implies, constrained to the
 * image allowlist.
 *
 * @param {string|null|undefined} filename - Client-supplied name.
 * @returns {string|null}
 */
export function imageExtensionFromFilename(filename) {
  const match = /\.([a-z0-9]{2,10})$/i.exec(String(filename ?? '').trim());
  if (!match) return null;
...
/**
 * Format a timestamp with a fixed UTC time for purchase/download receipts.
 *
 * @param {string|Date|null|undefined} input
 * @returns {string} e.g. `3 Sep 2026, 9:30 PM UTC`, or '—'.
 */
export function formatDateTimeUTC(input) {
  ...
  return `${date.getUTCDate()} ${months[date.getUTCMonth()]} ${date.getUTCFullYear()}, ${hour12}:${minutes} ${period} UTC`;
}
