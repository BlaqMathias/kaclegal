 ...
 * publication. If `file_path` is omitted from the body the existing file is kept;
 * if a new path is supplied, the row is updated first and only then is the old
 * file removed, so a failed update can't destroy the file it still points at.
 *
...
export async function PUT(request, { params }) {
  ...
  if (!normalized.ok) {
    ...
    // with it. That file now belongs to no row, so discard it — but never touch
    // `existing.file_path`, which the unchanged row still points at.
    const discarded = await discardUnusedReplacement(body, existing);

    return NextResponse.json(
      { ok: false, fieldErrors: normalized.fieldErrors, fileDiscarded: discarded },
      { status: 400 },
    );
  }
    ...
    return NextResponse.json(
      {
        ok: false,
        error: discarded
          ? 'Could not save your changes. The newly uploaded file was discarded — please attach it again.'
  ...
  // The row now points at the new file, so the superseded one is safe to remove.
  // `data` is the row as the database returned it, which is the authoritative
  // answer to "what does this publication point at now".
...
/**
 * DELETE /api/admin/publications/[id] — delete a publication and its file.
 *
 * The row goes first, then the object in Storage. Deleting the file first would
 * risk a published row pointing at nothing if the row delete then failed.
 *
...
export async function DELETE(_request, { params }) {
  ...
  // Best-effort: a leftover object in a private bucket is untidy, not unsafe, and
  // must not turn a successful delete into an error for the admin. The reference
  // check is what makes this safe if two rows ever shared a file.
